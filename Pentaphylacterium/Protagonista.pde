class Protagonista { 
  ...
  void verificarObstaculos(int[][] obstaculos, int numElemInt) {
    ...
    for (int n = 0; n < numElemInt; n++) {
      if (obstaculos[0][n] -20 < px +12 && obstaculos[0][n] +32 > px +12 && obstaculos[1][n] +26 > py +24 && obstaculos[1][n] -26 < py+24+vel) { // deteccion de elementos abajo
        cnt ++;
      } 
      if (obstaculos[0][n] -20 < px +12 && obstaculos[0][n] +32 > px +12 && obstaculos[1][n] -26 < py +24 && obstaculos[1][n] +26 > py+24-vel) { // deteccion de elementos arriba
        cnt2 ++;
      }
    }
    for (int m = 0; m < numElemInt; m++) {
      if (obstaculos[0][m] -16 < px+12+vel && obstaculos[0][m] +32 > px+12+vel && obstaculos[1][m] +20 > py+24 && obstaculos[1][m] -26 < py+24) {// deteccion de elementos derecha
        cnt3 ++;
      } 
      if (obstaculos[0][m] +30 > px+12-vel && obstaculos[0][m] -32 < px+12-vel && obstaculos[1][m] +20 > py+24 && obstaculos[1][m] -26 < py+24) {// deteccion de elementos izquierda
        cnt4 ++;
      }
    }
